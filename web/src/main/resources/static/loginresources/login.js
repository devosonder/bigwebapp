Ext.onReady(function () {
        ...
        form.submit({
            ...
            success: function (form, action) {
                ...
                Ext.toast({
                    ...
                    slideInDuration: 100,
                    hideDuration: 100,
                    bodyStyle: {background: "red", color: "white", textAlign: "center"}
                });
            }
        });
    }
    ...
    loginPanel = new Ext.form.Panel({
        ...
        fieldDefaults: {msgTarget: "qtip"},
        items: [{
            fieldLabel: i18n.user_loginname, name: "username", allowBlank: false, listeners: {
        ...
        dockedItems: [{
            xtype: "toolbar", dock: "bottom", items: ["->", {
                text: i18n.login_passwordreset_forgotpw, handler: function () {
                    loginPanel.hide();
                    passwordResetPanel.show();
                    passwordResetPanel.getForm().setValues({loginNameOrEmail: loginPanel.getValues().username});
                    passwordResetPanel.getForm().findField("loginNameOrEmail").focus();
                }
            }]
        }, {
            xtype: "toolbar", dock: "bottom", items: ["->", {
                text: i18n.login_as_admin, glyph: 59393, handler: function () {
                    var form = this.up("form").getForm();
                    form.setValues({username: "admin@bigwebapp.com", password: "admin"});
                    submitForm();
                }
            }, {
                text: i18n.login, glyph: 59393, handler: function () {
                    submitForm();
                }
            }]
        }]
    ...
    var passwordResetPanel = new Ext.form.Panel({
        ...
        items: [{xtype: "displayfield", value: i18n.login_passwordreset_description, padding: 10}, {
            xtype: "textfield",
        ...
        dockedItems: [{
            xtype: "toolbar", dock: "bottom", items: ["->", {
                text: i18n.login_passwordreset_sendlink, formBind: true, handler: function (btn) {
                    Ext.Ajax.request({
                        url: "passwordresetEmail", params: btn.up("form").getValues(), success: function (response) {
    ...
    var centerContainer = new Ext.container.Container({
        ...
        region: "center",
        layout: "center",
        items: [{xtype: "container", layout: "vbox", width: 600, items: [loginPanel, passwordResetPanel]}]
    });
